...
public class InputVerifier {
	public boolean isCorrectMember (String Name, String PersonalNumber, String UserName, String Password, Member Member){
		...
		return (isCorrectMemberName(Name) && isCorrectMembePersonalNumber(PersonalNumber, Member)&& isCorrectMemberUserName(UserName, Member) && isCorrectMemberPassword(Password));
	}
	...
	private boolean isCorrectMembePersonalNumber(String PersonalNumber, Member Member) {
		if (PersonalNumber.length()<6 || !isDate (PersonalNumber.substring(0, 6))) {
			alertWindow(AlertType.ERROR, "ERROR!", "Cannot add member!", "First six charecters in PERSONAL NUMBER should be the birth date");
			return false;
		}
		if (Member==null) {
			for (Member m : Main.yachtClub.getMemberList()) {
